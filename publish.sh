...
do
    ...
    PACKAGE_NAME=$(printf '%s\n' "${filename%-macos10.15-x86_64.tar.xz}" | sed 's|\(.*\)-\(.*\)|\1|')

    PACKAGE_VERSION=$(printf '%s\n' "${filename%-macos10.15-x86_64.tar.xz}" | sed 's|\(.*\)-\(.*\)|\2|')

    PACKAGE_BIN_SHA=$(sha256sum "package/$filename" | cut -d ' ' -f1)

    PACKAGE_BIN_URL="https://github.com/leleliu008/uppm-package-repository-macos10.15-x86_64/releases/download/${RELEASE_VERSION}/${filename}"
...
run tar vxf package/gtar-*-macos10.15-x86_64.tar.xz -C "$TEMP_DIR" --strip-components=1
run tar vxf package/gzip-*-macos10.15-x86_64.tar.xz -C "$TEMP_DIR" --strip-components=1
run tar vxf package/xz-*-macos10.15-x86_64.tar.xz   -C "$TEMP_DIR" --strip-components=1
